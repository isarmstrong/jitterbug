...
describe('MetricsProcessor', () => {
    ...
    describe('Event Loop Tracking', () => {
        it('should track event loop lag', async () => {
            ...
            mockDate.mockReturnValueOnce(1000)  // Initial time
        ...
        it('should detect event loop lag spikes', async () => {
            ...
            // Normal lag
            mockDate.mockReturnValueOnce(1000)
                .mockReturnValueOnce(1010);
            results.push(await processor.process(entry));

            // High lag
            mockDate.mockReturnValueOnce(1100)
                .mockReturnValueOnce(1200);
            results.push(await processor.process(entry));

            // Back to normal
            mockDate.mockReturnValueOnce(1300)
                .mockReturnValueOnce(1310);
            results.push(await processor.process(entry));
